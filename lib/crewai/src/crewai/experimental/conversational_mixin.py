...
def _graph_method(handler: Callable[..., Any], **roles: Any) -> FlowMethodDefinition:
    """One built-in graph entry, as the same code ref the DSL projection emits."""
    return FlowMethodDefinition(do=_method_action(handler), **roles)


def _builtin_graph() -> dict[str, FlowMethodDefinition]:
    """The graph entries a conversational flow needs to run a turn.
...
class _ConversationalMixin:
        ...
        **kickoff_kwargs: Any,
        ...
        Raises:
            ValueError: If the flow is not conversational. Without this the
                turn is a silent no-op: the graph has no route handlers, the
    ...
    def _conversation_flow_definition(self) -> FlowDefinition:
        """Return the definition that describes this flow's graph.
    ...
    def _extend_definition(self, definition: FlowDefinition) -> FlowDefinition:
        """Add the built-in conversational graph a declaration cannot inherit.
        ...
        missing = {
            name: method
            for name, method in _builtin_graph().items()
